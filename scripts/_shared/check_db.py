...
⚠ DEPRECATED 2026-05-01: this script targets `market_in.candles_1min`, which
was DROPPED in migration 022 (replaced by split fact tables `fact_equity` /
`fact_index` / `fact_futures`). The new fact tables require `freq_id` +
`endpoint_id` FK lookups that this script doesn't perform.

For ad-hoc DB verification post-redesign, use:
  - playground/check_redesign_phase_c.py  (full Phase A-F integrity check)
  - scripts/verify_political.py           (alt_political_us audit + flagship query)

This file is kept as historical reference. Do not run.
"""
raise SystemExit(
    "batch_test_db.py is deprecated post-redesign. See docstring for replacements."
)
...
def run_tests(keep: bool = False) -> int:
    ...
    with engine.connect() as conn:
        ...
        for i in range(5):
            ...
            conn.execute(text("""
                INSERT INTO market_in.candles_1min
        ...
        eq_count = conn.execute(text("""
            SELECT count(*) FROM market_in.candles_1min
        ...
        if result:
            print(f"  bar_time={result[0]}, close={result[1]}, symbol={result[2]}, "
                  f"exchange={result[3]}, market_in={result[4]}, country={result[5]}")
        ...
        hypertables = conn.execute(text("""
            ...
            WHERE hypertable_schema = 'market_in'
        ...
        if keep:
            ...
            conn.execute(text("DELETE FROM market_in.candles_1min WHERE source = 'test'"))
