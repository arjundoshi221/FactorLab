...
def find_equities(
    ...
    """Build ``{trading_symbol: instrument_record}`` lookup for tradeable equities.

    Includes both ``EQ`` (regular rolling segment) and ``BE`` (Book Entry /
    new-listing surveillance) instrument types. BE names are tradeable but
    sit in surveillance for ~6-12 months post-listing or post-demerger
    (e.g. Vedanta demerger spinoffs currently in nifty500).
    """
    return {
        i["trading_symbol"]: i
