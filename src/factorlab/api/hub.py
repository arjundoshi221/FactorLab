...
class HubRepository:
    ...
    def get_overview(self, *, now: datetime | None = None) -> HubOverview:
        ...
        for row in metadata:
            ...
            table = HubTable(
                ...
                today_status="not_configured" if profile is None else "unknown",
                status_reason=(
                    "Add this table to the dashboard profile registry to evaluate its dates and health."
                    if profile is None
        ...
        summary = HubDatabaseSummary(
            ...
            populated_tables=sum(item.stored_rows > 0 for item in tables),
            stored_rows=sum(item.stored_rows for item in tables),
            bytes_on_disk=sum(item.bytes_on_disk for item in tables),
            healthy_tables=sum(item.today_status == "healthy" for item in tables),
            attention_tables=sum(item.today_status in attention for item in tables),
        )
        return HubOverview(
    ...
    def _table_status(
        ...
        if profile.status_policy == "not_expected":
            return "not_expected", "This table does not have a daily row-count expectation."
