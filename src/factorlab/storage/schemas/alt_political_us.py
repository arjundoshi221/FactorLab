"""alt_political_us schema — US Congress STOCK-Act trades, lobbying, contracts,
campaign donations, bills, hearings.

Per-country schema: future India / EU political data lands in sibling schemas
(alt_political_in, alt_political_eu) with their own jurisdiction-specific dims.
...
Audit trail (gzipped bodies + URL + status) lives in the unified
`audit.raw_archive` table — see factorlab.storage.schemas.audit.
...
# ---------------------------------------------------------------------------
# alt_political_us.legislators — one row per real human (bioguide_id PK)
# ---------------------------------------------------------------------------
legislators = Table(
    "legislators",
    metadata,
...
# ---------------------------------------------------------------------------
# alt_political_us.legislator_terms — SCD: chamber/state/district/party per term
# ---------------------------------------------------------------------------
legislator_terms = Table(
    ...
    Column("party", String(50), nullable=True),
...
# ---------------------------------------------------------------------------
# alt_political_us.legislator_fec_ids — multiple FEC IDs per real person
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.legislator_aliases — manual override curation (mig 024)
# Stage 0 priority for the bioguide matcher: alias_normalized -> bioguide_id.
# Used when the strict matcher cannot bridge a name (e.g. Van Taylor files as
# 'Nicholas V. Taylor' but bioguide stores him as first='Van').
# ---------------------------------------------------------------------------
legislator_aliases = Table(
    "legislator_aliases",
    metadata,
    Column("country_code", String(2), ForeignKey(COUNTRY_FK),
           primary_key=True, server_default="US"),
    Column("alias_normalized", String(200), primary_key=True,
           comment="Lowercase, stripped of punctuation; matches matcher input."),
    Column("bioguide_id", String(7),
           ForeignKey(f"{SCHEMA}.legislators.bioguide_id", ondelete="RESTRICT"),
           nullable=False),
    Column("source", String(50), nullable=False,
           comment="'manual_pdf_review' | 'historical_yaml' | 'curated' ..."),
    Column("confidence", String(15), nullable=False,
           comment="'verified' | 'high' | 'medium'"),
    Column("verified_via", Text, nullable=True,
           comment="Pointer to evidence: PDF path, URL, doc_id, etc."),
    col_created_at(),
    CheckConstraint("confidence IN ('verified','high','medium')", name="confidence"),
    schema=SCHEMA,
    comment="Manual / curated alias overrides. Stage 0 of bioguide matcher.",
)
Index("ix_legislator_aliases_bioguide", legislator_aliases.c.bioguide_id)


# ---------------------------------------------------------------------------
# alt_political_us.committees — committee dimension (current + historical)
...
# ---------------------------------------------------------------------------
# alt_political_us.committee_assignments — point-in-time committee membership
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.committee_sector_map — sectoral relevance per committee
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.asset_type_codes — House Clerk's 48 codes
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.lda_issue_codes — 79 LDA issue codes
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.lda_government_entities — 257 lobbyable orgs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.fec_committees — FEC PACs + campaign committees
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.contract_aliases — recipient_name → ticker resolution
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.lobby_client_aliases — LDA client_name → ticker
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.legislator_trades — STOCK Act PTRs (House + Senate)
# ---------------------------------------------------------------------------
legislator_trades = Table(
    ...
    # Filing identity — endpoint_id replaces freeform `source` String (mig 023)
    Column("endpoint_id", Integer,
           ForeignKey("ref.data_endpoints.id"), nullable=False,
           comment="Which feed produced this row. JOIN to ref.vendors via FK chain."),
    ...
    Column("filer_type", String(20), nullable=True,
           comment="'self' | 'spouse' | 'dependent_child' | 'joint' | 'junior'. "
                   "NULL when source doesn't disclose (strict-NULL policy)."),
    Column("transaction_type", String(20), nullable=True,
           comment="'purchase' | 'sale_full' | 'sale_partial' | 'exchange'. "
                   "NULL when source doesn't disclose (e.g. paper PTRs pre-OCR)."),
    # Asset
...
# ---------------------------------------------------------------------------
# alt_political_us.gov_contracts — federal contract awards
# ---------------------------------------------------------------------------
gov_contracts = Table(
    "gov_contracts",
    metadata,
...
# ---------------------------------------------------------------------------
# alt_political_us.lobbying_filings — LDA quarterly filings
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.lobbying_activities — per-filing per-issue rows
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.lobbying_activity_targets — M:N activity ↔ government_entity
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.lobbying_activity_lobbyists — M:N activity ↔ person
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.campaign_donations — FEC schedule_a contributions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.bills — Congress.gov bills
# ---------------------------------------------------------------------------
bills = Table(
    "bills",
    metadata,
...
# ---------------------------------------------------------------------------
# alt_political_us.bill_sponsors — M:N bills ↔ legislators with role
# ---------------------------------------------------------------------------
bill_sponsors = Table(
    "bill_sponsors",
    metadata,
...
# ---------------------------------------------------------------------------
# alt_political_us.bill_committees — bills referred to / acting committees
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# alt_political_us.bill_actions — full action history per bill
# ---------------------------------------------------------------------------
bill_actions = Table(
    "bill_actions",
    metadata,
...
# ---------------------------------------------------------------------------
# alt_political_us.hearings — committee hearings
# ---------------------------------------------------------------------------
hearings = Table(
    "hearings",
    metadata,
...
# ---------------------------------------------------------------------------
# alt_political_us.hearing_witnesses — witnesses listed for each hearing
# ---------------------------------------------------------------------------
...
# NOTE: alt_political.raw_archive was dropped in migration 016. Audit lives in
# `audit.raw_archive` (single unified table). Import via:
#     from factorlab.storage.schemas.audit import raw_archive
