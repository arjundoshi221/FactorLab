...
INNER JOIN meta.migration_political_trade_enrichment AS p FINAL
   ...
   AND p.legacy_key = f.filing_id AND p.approved_at IS NOT NULL
LEFT JOIN (
...
INNER JOIN meta.migration_political_trade_enrichment AS p FINAL
   ...
   AND p.legacy_key = t.trade_key AND p.approved_at IS NOT NULL
INNER JOIN meta.migration_id_crosswalk AS x FINAL
   ...
   AND x.legacy_key = t.trade_key AND x.target_kind = 'political_trade'
...
FROM (
    SELECT
        ...
        any(t.source) AS source,
        any(coalesce(t.ticker, t.asset_name_raw)) AS alias_value,
        concat('{"trade_key":"', t.trade_key, '"}') AS context_json,
...
WITH
    snapshot_index AS (
        ...
        FROM (
            ...
            FROM {{source_database}}.alt_political_committee_memberships
        )
    ),
    observed AS (
        SELECT
...
SELECT
    ...
    p.ingested_at, greatest(p.version, cm.version, lm.version)
FROM periods AS p
...
INNER JOIN meta.migration_id_crosswalk AS cm FINAL
   ...
   AND cm.legacy_key = p.committee_id AND cm.target_kind = 'entity'
...
INNER JOIN meta.migration_id_crosswalk AS lm FINAL
   ...
   AND lm.legacy_key = p.bioguide_id AND lm.target_kind = 'entity'
...
WHERE target.committee_id = '' OR greatest(p.version, cm.version, lm.version) > target.version;
