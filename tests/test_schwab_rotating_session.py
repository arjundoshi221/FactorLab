...
def test_session_retries_safe_request_once_after_token_rotation(monkeypatch):
    session = SchwabSession("old-token")
...
def test_session_does_not_retry_order_after_401(monkeypatch):
    session = SchwabSession("old-token")
    available_tokens = iter(["old-token", "new-token"])
    monkeypatch.setattr(
        "factorlab.sources.schwab.client.get_secret",
    ...
    monkeypatch.setattr(requests.Session, "request", fake_request)
    response = session.post("https://example.test/trader/v1/orders", json={"quantity": 1})
